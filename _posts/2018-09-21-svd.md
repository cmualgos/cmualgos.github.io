...
3. The _trace norm_ is given by $\|M\|_{tr} := \sum_\ell | \sigma_\ell
|$, and hence equals $\| \mathbf{\sigma} \|_1$.

4. In general, we can define for any $p$ the [_$p$-Schatten
norm_](https://en.wikipedia.org/wiki/Schatten_norm) which is given by
$\|\mathbf{\sigma}\|_p$.

5. Given any vector norm $\|\cdot \|$, we can define the _induced_ or
_operator_ norm $\| A \| := \sup \frac{\|Ax\|}{\|x\|}$. Often one tends
to use the term _operator_ norm just for the case when we use the
Euclidean vector norm, in which case it just equals the spectral norm.
...
values, and hence is invertible, with $\Sigma^{-1} = diag(1/\sigma_1,
1/\sigma_2, \ldots, 1/\sigma_r)$. Moreover, we get the invaluable
...
iteration](https://en.wikipedia.org/wiki/Arnoldi_iteration) [Lanczos'
